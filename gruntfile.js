...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    shell: {
      ...
      run: {
        command: [
          'docker rm -f portainer_sapan',
          'docker run -d -p 9004:9000 -v $(pwd)/dist:/app -v $(pwd)/userdata:/data -v /var/run/docker.sock:/var/run/docker.sock:z --name portainer_sapan centurylink/ca-certs /app/portainer-linux-amd64 --no-analytics -a /app'
        ].join(';')
      }
    },
    replace: {
